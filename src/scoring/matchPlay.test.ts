...
describe('generateBracket', () => {
  it('returns empty when fewer than 2 opt-ins', () => {
    ...
    expect(generateBracket([p('a', 10)], 'A')).toEqual([]);
  });

  it('produces a 2-bracket with one final match for 2 opt-ins', () => {
    const matches = generateBracket([p('a', 10), p('b', 12)], 'A');
    expect(matches.length).toBe(1);
    expect(matches[0]).toMatchObject({ id: 'm1-0', divisionCode: 'A', round: 1, slot: 0, playerASaId: 'a', playerBSaId: 'b' });
  });
  ...
  it('produces an 8-bracket for 8 opt-ins, no byes', () => {
    ...
    const matches = generateBracket(players, 'A');
...
describe('propagateAll', () => {
  ...
  it('clears downstream winner when an earlier-round winner is changed', () => {
    ...
    let matches = generateBracket(players, 'A');
    ...
    const finalMatch = after.find((m) => m.id === 'm3-0')!;
...
describe('matchesByRound', () => {
  it('groups by round and sorts slots ascending within each round', () => {
    const matches: Match[] = [
      { id: 'm2-1', divisionCode: 'A', round: 2, slot: 1 },
      { id: 'm1-3', divisionCode: 'A', round: 1, slot: 3 },
      { id: 'm1-0', divisionCode: 'A', round: 1, slot: 0 },
      { id: 'm2-0', divisionCode: 'A', round: 2, slot: 0 },
      { id: 'm1-1', divisionCode: 'A', round: 1, slot: 1 },
      { id: 'm1-2', divisionCode: 'A', round: 1, slot: 2 },
      { id: 'm3-0', divisionCode: 'A', round: 3, slot: 0 },
    ];
    const grouped = matchesByRound(matches);
    expect(grouped.length).toBe(3);
    expect(grouped[0].map((m) => m.id)).toEqual(['m1-0', 'm1-1', 'm1-2', 'm1-3']);
    expect(grouped[1].map((m) => m.id)).toEqual(['m2-0', 'm2-1']);
    expect(grouped[2].map((m) => m.id)).toEqual(['m3-0']);
  });
});
